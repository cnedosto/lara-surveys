...
<div>
    <label for="{{$attributes->whereStartsWith('wire:model')->first()}}" class="block text-sm font-medium leading-6 text-gray-900">{{$label}}</label>
    ...
    @error($attributes->whereStartsWith('wire:model')->first())
    <p class="mt-2 text-sm text-red-600" id="email-error">Not a valid <span class="lowercase">{{$label}}</span>.</p>
    @enderror
</div>